...
```{r funct_resp1, echo=FALSE, fig.margin=TRUE, fig.height=5, fig.cap="Type I"}
prey <- seq(0, 60, 5)
type1 <- 0.15 * prey
...
Notice this is in the form $y = \beta x + \alpha$. We can easily find out the unknown parameters using simple regression which will tell us the intercept and slope. 
...
```{r funct_resp2, echo=FALSE, fig.margin=TRUE, fig.height=5, fig.cap="Type II"}
prey <- seq(0, 60, 5)
...
```{r funct_resp, echo=FALSE, fig.margin=TRUE, fig.height=5, fig.cap="All types"}
prey <- seq(0, 60, 5)
type1 <- 0.15 * prey
type2 <- (prey*0.7)/(1+(prey*0.7*0.15))
