...
output:
  html_document:
    ...
    toc: TRUE
    code_download: TRUE
subtitle: 'Lecture 2: multiple regression'
editor_options:
...
kable(anova(lm(Height ~ Sex, survey)), digits = 2)
```
...
* Identity of person is not the main question
...
* Predictor variables can be categorical (called factors in R), like exercise in the previous example
* Can have two or more sub-categories (called levels in R)
  * E.g. sex: male/female, level of drug: low/medium/high  

Typically, the relationship between a continuous response variable & categorical predictors is analysed using a special case of statistical models:  
...
Report the degrees of freedom, the F ratio and the P value like
...
|:---------------------:|:--:|:--:|:--:|:-:|---|
| Variance among groups | SSR | number of levels - 1 | $\frac{SSR}{df}$ | $\frac{MSR}{MSE}$ |   |
| Variance within group | SSE | number of observations - 2 | $\frac{SSE}{df}$|   |   |
...
* `Estimate` column shows the **difference in means**
...
* Need post-hoc tests like Tukey’s Honest Significant Differences (not covered in this module)
...
The random slopes model has in interaction term. In R this is coded as either `lm(Height ~ Wr.Hnd * Sex, survey)` or `lm(Height ~ Wr.Hnd + Sex + Wr.Hnd:Sex, survey)`.
