...
If you've answered no to any of the above questions, then continue on. Some of you may already have R and RStudio installed from previous modules - that's great! Check that your version and packages are all up to date (Under the Packages tab, click Update).
...
![](https://raw.githubusercontent.com/jacintak/biostats/master/docs/media/CRAN%20site.png)
...
R is just one of *many* programming languages but R is specially designed for doing statistics. Hence it is widely taught in statistic classes. There are other statistics programs (even Excel can do basic statistical tests) but these are normally "point-and-click" programs where you click a button and magic happens. The advantage of R is that it is an open source software where you can *see* how your data is being manipulated. Which means it's transparent, you can check whether the statistics is done correctly and is easier to see how statistical theory is being applied. It's no longer magic but *science*.
...
We can customise and expand the functionality of R by installing more packages, which are made by people and distributed freely. For example, I wrote this document in RStudio using an additional package called `Rmarkdown` (neat right?) which lets me make HTML, word or PDF files of text, code and figures or tables. These packages will need to be **called** when you open R to activate them or R will not use them. Though, where possible we will be sticking to base R.

```
# setup MASS 
library(MASS) # If I wanted to use the package MASS, I must enter and run the library() function at the start of my script
```
...
![](https://raw.githubusercontent.com/jacintak/biostats/master/docs/media/RStudio%20console.png)
...
![](https://raw.githubusercontent.com/jacintak/biostats/master/docs/media/setwd.png)
...
![](https://raw.githubusercontent.com/jacintak/biostats/master/docs/media/open%20script.png)

Let's add some code to this script. Copy and paste code from below into your script. 
...
![](https://raw.githubusercontent.com/jacintak/biostats/master/docs/media/Run.png)
...
```{r}
small_sleep <- read.delim("../2020/drop in 1/small_sleep.txt")
small_sleep
```
...
```{r}
mammal_sleep <- read.csv("../2020/drop in 1/mammal_sleep.csv")
```
...
![](https://raw.githubusercontent.com/jacintak/biostats/master/docs/media/clip.png)
...
The base R function to make a plot is `plot(y axis ~ x axis, data = data frame)`. You can customise all components of the graph - try looking up how to change the point style.
...
```
I won't tell you how but by the end of the module you should be able to make figures that are suitable for scientific reports - not implying that the figure above is a standard. 
...
**Answer these questions to check your understanding. Click code to show the answer**  
