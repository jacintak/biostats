...
H0: There is no difference in the number of prey eaten with prey density between predator experience  
H1: There is a difference in the number of prey eaten with prey density between predator experience 
...
In other words, we generally want to know if the change in predator experience *improves* the number of prey captured. If we used the original alternative hypothesis then we would accept that hypothesis even if the number of prey captured decreased.
...
quiz(
  question(
    "Which of these variables are we changing to compare different predator experience?",
...
quiz(question_text(
  ...
  question_text(
    ...
    answer("2", correct = TRUE, message = "Prey density and predator experience"),
    allow_retry = TRUE
  )
)
```
...
| predator_experience            	| character        	| `naive` for no experience or `experienced`	for a experienced predator |
...
```{r radio1, echo = FALSE}
question_radio(
  "What should be entered in the cell under the column predator_experience when using a predator without experience",
