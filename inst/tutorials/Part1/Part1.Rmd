...
In our experiment we have 1 predator moving at a constant speed. These are constants that does not change - doing so would violate the assumptions of our model or turn these parameters into variables.
...
| prey density              	| numeric          	| NA          	|
| replicate                     	| numeric          	| NA          	|
| prey eaten           	| numeric          	| NA          	|
| foraging strategy            	| character        	| `no_lid` for no lid or `yes_lid`	for a jar with a lid |
...
```{r radio1, echo = FALSE}
question_radio(
  "What should be entered in the cell under the column foraging_strategy when using a jar without a lid",
  answer("no_lid", correct = TRUE),
  answer("yes_lid"),
  allow_retry = TRUE
)
```
...
Save your data sheet **as a csv file** (in Excel, click "Save As" and select csv file type). Your file name should be your bench number and seats (no spaces) and practical session separated by `_`. e.g. `Bench134_AM.csv` for Bench 1 seats 3 & 4 in the morning session.
