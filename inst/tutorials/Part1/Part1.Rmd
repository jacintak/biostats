...
#subtitle: "2022"
description: "2022 BYU22S01 Practical 4"
---
...
This worksheet is an interactive document run through RStudio. It is made using the R package `learnr`. The worksheet has interactive code chunks that you can use to enter R code and quizzes to test yourself -- which makes this more fun than a regular word document or PDF and lets you practice R coding in a structured environment.

You can interact with this worksheet through the Tutorial tab in RStudio. This window can be resized and expanded. 

> I recommend clicking the pop out button (in between the home icon and the stop icon) to open the tutorial in a separate window. 

***

**Code chunks**  
Code chunks are independent of the main RStudio Environment. Anything you type here will not be saved to memory. 

Here's a simple exercise with an empty code chunk provided for entering the answer. Click __Run Code__ to run the code, keyboard shortcuts will work too.  
__Start Over__ resets the code chunk.  
The __Hints__ button will tell you a hint. If you are really stuck, the last hint may be the solution.
...
**Quizzes**  
**Quizzes here are not part of the CA.** The questions are meant to check you understand the material and your answers are not visible to anyone else. If you can answer the quizzes correctly, then the CA will be no trouble for you.
...
question_radio("Statistical models aim to accurately and precisely describe reality as it is",
  answer("False", correct = TRUE, message = "How do you know that what you percieve is true? Or, how you you know you are not living in a computer simulation?"),
...
<h3 class="panel-title">Continuous Assessment</h3>
  ...
  <div class="panel-body">
I recommend finishing the CA *after* Part B and *before* Part C. The CA is due at the end of the prac. 
...
## Working with a parameterised model

Once we've collected data, we can use our parameterised Type II functional response to make **predictions** about our predator-prey system. 

For example, how many counters would a student pick up if there were 50 counters?

If you know the handling time and the search rate then making predictions requires substituting all your known information into the equation and solving linear algebra! 

Using either the non-linearised or the linearised model works. Remember that the linearised model uses the **inverse** of prey density and prey captured as the predictor and response variables so don't forget to convert inverted data in to non-inverted data and vice versa.
...
Foraging strategy becomes a **second predictor variable** that takes two values:
...
```{r quiz3}
quiz(caption = "Get all these answers correct before moving to the next section.",
  question_text(
    paste("How many treatments are we using?"),
    answer("5", correct = TRUE, message = random_praise()),
    allow_retry = TRUE,
    incorrect = random_encouragement()
  ),
  question_text(
...
> Now do the CA and hand it in for marking. Then do Part C. Start your experiment after you've worked through Part C.
...
Tidy data is set of principles for organising data sheets and filling in data. It is designed in a way to make data analysis easier and reduces the amount of work required to *prepare* your data for analysis later on.
...
<h3 class="panel-title">Info!</h3>
  ...
  <div class="panel-body">
These guidelines match how R reads data from a spreadsheet. Thus, following these best practices will prevent errors when importing data into R. 
...
|-------------------------------	|------------------	|-------------	|
| total_time 	                    | numeric          	| in minutes (i.e. `1`)	|
| prey_density              	| numeric          	| NA          	|
| replicate                     	| numeric          	| NA          	|
| prey_eaten           	| numeric          	| NA          	|
| foraging_strategy            	| character        	| `no_lid` for no lid protocol or `yes_lid`	for a jar with a lid protocol |
...
<h3 class="panel-title">Warning!</h3>
  ...
  A space counts as a character so `"cat"`, `"cat "` and `" cat"` would be read as three unique inputs. This kind of human error can mess up your analysis but is very common.
 </div>
</div>
...
question_radio(
  "What should be entered in the cell under the column foraging_strategy when simulating an inexperienced predator?",
...
question_radio(
  "What should be entered in the cell under the column foraging_strategy when simulating an experienced predator?",
  answer("no_lid", correct = TRUE, message = random_praise()),
  answer("yes_lid"),
  allow_retry = TRUE,
  incorrect = random_encouragement()
)
...
In data science, data usually needs to be processed before we can use it. The data needs to be checked and validated. Are all the fields entered correctly? Attention to detail is important in case it introduces mistakes in our data that may bias our analyses!
...
Save your data sheet **as a csv file** (click "Save As" and select csv file type). No other file types are accepted. **Only one person in the group needs to upload a file**.
...
* Does your data sheet fully conform to the guidelines above?
    * Foraging strategy conforms to guidelines?
    * Correctly formatted numeric columns?
