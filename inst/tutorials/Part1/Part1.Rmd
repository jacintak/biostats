...
Part C: Collecting data (**1 hour**) 

This is the main activity of the practical that will take the most time and you can't rush/skip it.
...
I recommend doing the CA *after* Part B. Give yourself 30 mins to do the CA but don't spend too long on it because you need to finish Part C.
...
This model was simple - maybe, too simple to be realistic. The killer T cell captured any and all pathogens touching it whereas in reality, a killer T cell may only target a few pathogens at a time or sequentially. This is also true for animal predators who hunt. Most predators need time to catch and process their food before their next meal.
...
There are multiple types of functional responses (labelled with Roman numerals: I - IV). Each of these models represents a *different hypothesis* about functional responses and mathematically describes a *different relationship* between our two variables (the killer T cell and the pathogen). The assumptions we make above are based on a **Type II functional response model** and is the one most appropriate for an infection response scenario. You can read about the full maths by running `vignette("TypeIImodels")`. We won't discuss the other types. 

The Type II functional response model is a good example of a general model - it has been used to describe animal predators eating prey (e.g. Holling's disc equations) or enzymes catalysing reactions (e.g. Michaelis-Menten equation). It doesn't matter what the mathematical symbols represent biologically, maths is a universal language that describes the underlying biological process. Here, we are applying it to an immune response scenario.
...
question_checkbox(
  "Which of these words describe our infection response model? Select all that apply.",
  answer("Complex"),
...
Experiments consist of variables that are *controlled* or *manipulated*. The manipulated variables are usually predictor variables. Controlled variables are other influential variables that may affect the response variable in ways that might mask or accentuate the effect of the predictor variable on the response variable.  
 
 > We control as many possible variables as possible to make sure what we observe is the true effect of the predictor variable of interest.   
...
**For the purpose of this practical, we will decide on 5 treatments of pathogen density. You are free to choose any 5 numbers from a range of 1 to 100.** 
...
Replication is really important to increase the accuracy and precision of our data and make sure our results are not due to random chance. It also reduces the chances of making a type 1 or type 2 error. 

> Generally, the more replication the better! Again, the trade-off is more work, time, effort or computing power.

**For the purpose of this practical, we will use 3 replicates of pathogen density.** Three is usually a minimum number of replications. In a real-world scientific study, you may see higher replications.  
...
In our experiment we have 1 killer T cell moving at a constant speed. These are constants that does not change - doing so would violate the assumptions of our model or turn these parameters into variables.
...
> Remember: keep it simple! A more complex statistical analysis cannot fix a issues arising from a badly designed experiment.
...
What happens if our killer T cell was more efficient? For example, vaccines provide a chance for the immune system to "learn" how to destroy pathogens (e.g. viruses). After this experience, immune systems become more efficient at destroying pathogens.   
**How would this aim affect $H_a$ and the estimates of $a$ and $T_h$?**  

The efficiency of the killer T cell is set as a variable in Scratch called `efficiency`. In the model, the duration of time a killer T cell waits while digesting is a random number between 1 and 5 that is multiplied by the value of `efficiency`. For example, if digesting time was 5 and efficiency was 1, then the effective digesting time is 5 * 1 = 5 seconds. The `efficiency` value is a constant parameter in our simulation - it doesn't change and applies to all runs of the simulation; changing this value changes the intended scenario.

_Let's call our original killer T cell an **unvaccinated individual** with an `efficiency` value of 1_. Now, consider a **vaccinated individual** with an `efficiency` value of 0.5. We have changed our constant parameter, it doesn't fundamentally change our model assumptions but becomes a new variable that is used to calculate the effective digesting time. 
...
quiz(
  question(
    "Which of these variables are we changing to compare vaccinated and unvaccinated individuals?",
  ...
  question_text(
    paste("How long (in seconds) would a killer T cell of a vaccinated individual pause in the simulation if digesting time was 3? Numbers only."),
    answer("1.5", correct = TRUE),
    allow_retry = TRUE,
    message = "What is the value of efficiency for a vaccinated individual and how is efficiency used in our model?"
  )
)
```
...
It's often more interesting to formulate a more specific alternative hypothesis than just the opposite of the null hypothesis if we hypothesise there's a *direction* to the effect of the new predictor variable on the response variable. We will use this one:

H1: Killer T cells of vaccinated individuals will destroy more pathogens than killer T cells of unvaccinated individuals. 

In other words, we generally want to know if the vaccine *improves* the immune response. If we used the original alternative hypothesis then we would accept that hypothesis even if the vaccine makes people *more susceptible* to disease. A) Yikes, B) it's not an informative or helpful conclusion if we were testing a new vaccine.
...
quiz(question_text(
    ...
    answer("30", correct = TRUE, message = "All other variables and constants stay the same"),
    allow_retry = TRUE
  ),
  question_text(
...
> Now do the CA - 9 questions, 1 mark for each question. 
...
|-------------------------------	|------------------	|-------------	|
| student number                	| NA          	    | for marking 	|
| total time 	                    | numeric          	| `1` in minutes, not seconds	|
| pathogen density              	| numeric          	| NA          	|
| replicate                     	| numeric          	| NA          	|
| pathogens destroyed           	| numeric          	| NA          	|
| vaccination status            	| character        	| `no` for unvaccinated or `yes`	for vaccinated |
...
Now you can do your experiment and record your data. 30 data points should take you about 30-40 minutes because that's how long it takes the simulation to run 30 times. Remember to save your data regularly.

```{r radio1, echo = FALSE}
question_radio(
  "What efficiency value is used in the simulation for an individual with the vaccination status 'no'?",
  answer("1", correct = TRUE),
  answer("0.5"),
  allow_retry = TRUE
)
...
quality_check <- function(x){
  ...
  if((sum(x[,6] != tolower(x[,6])) != 0) == TRUE){pass <- 0} # vaccine
  
  # check complete
  if(sum(complete.cases(x)) != 30){pass <- 0} # missing values?

  return(message(ifelse(pass==1, "Passed", "Failed")))
}
```

You see that the function will take your data, then check whether the information is formatted correctly and whether all 30 observations are filled. Because the function uses square bracket subsetting for identifying columns, the order of the columns is very specific. Finally the data sheets gets assigned a 1 if it passes and a 0 if it fails with a little Pass/Fail message.
...
Save your data sheet **as a csv file** (in Excel, click "Save As" and select csv file type). Your file name should be your student number and practical session separated by `_`. e.g. `12345678_AM.csv`.

Upload your csv file to the assignment link provided on Blackboard.
...
Now that you have your data, the next step is to explore your data graphically. We've covered how to import data and graph data in R in previous practicals. You can try to import your data and graph your data. We will do this next prac as well.

Graphing data is a great way of checking for quick and easy to spot errors or identifying emerging patterns. 
