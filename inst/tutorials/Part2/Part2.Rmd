...
**Code chunks**  
Code chunks are independent of the main RStudio Environment. Anything you type here will not be saved to memory, but you won't need to. 

Here's a simple exercise with an empty code chunk provided for entering the answer. Click __Run Code__ to run the code, keyboard shortcuts will work too. __Start Over__ resets the code chunk. The __Hints__ button will tell you a hint. If you are really stuck, the last hint may be the solution.
...
You do not need to highlight the entire code to run it. Place your cursor on the code line you want to run, then press Ctrl (Cmd on Macs) + Enter. </div>
</div>
...
Although you can come back to this tutorial at any time, I recommend taking your own notes for your reference. 
...
<h3 class="panel-title">Discussion</h3>
  ...
  <div class="panel-body">
These are discussion questions that you should think about or write down an answer. </div>
</div>

Click **Next Topic** to progress to the practical instructions.  
You can navigate between topics using the buttons or the menu. You can access the menu if minimised by clicking the the title "Statistical Modelling Part 2". 

<div class="panel panel-danger">
<div class="panel-heading">
<div class="panel-title">
You must complete all the exercises to move ahead.  
Do not skip exercises -- This is the "practice" of the practical. </div>
</div>
</div>
...
We have covered all of these concepts in the lectures or previous practicals. This prac will reinforce and revise these concepts and allows you to practise them under supervision of the demonstrators.

<div class="panel panel-primary">
<div class="panel-heading">
<div class="panel-title">
I recommend taking your time and making sure you understand the content and are up to date with the module. This practical synthesises and integrates *everything* up to this point. There is no new material. </div>
</div>
</div> 
...
<div class="panel-heading">
<div class="panel-title">
I do not recommend skipping the theory before attempting the CA -- the theory section allows you to practice all the steps you need for analysing your data. 

If you have no trouble with the example dataset, then you'll have no trouble with the CA or any experimental design.</div>
</div>
</div>
...
<div class="panel-heading">
<div class="panel-title">
You can **and should** easily fix the above mistakes yourself! </div>
</div>
</div> 
...
<h3 class="panel-title">Danger!</h3>
  ...
  <div class="panel-body">
Do not take the code here for granted. One does not simply copy code from somewhere and blindly expect it to work.
...
We expect you to know what the code means, describe what it's doing and know how to use it properly. </div>
</div>
...
### Talk to a rubber duck
 ...
 > Programming requires a clear idea of what you want to achieve. You need to do the thinking for the computer.
 
One of the hardest thing about troubleshooting someone else's programming is that it's not clear what the problem is. When describing your problem, be comprehensive & give context. The more information you provide, the more information and context people have to help you.

If something isn't working, can you articulate what you aim to do? What you expect to see if it worked? What you did leading up to this point? And what you think is happening to cause the error? This is called the **rubber duck method** -- like you are explaining your problems to a rubber duck. Notice, it's exactly like the Scientific Method.
...
<div class="panel panel-primary">
  <div class="panel-heading">
<h3 class="panel-title">Let me google that for you</h3>
  </div>
  <div class="panel-body">
There's no shame in googling everything -- we do too and finding things out yourself part of learning to work independently or troubleshooting.

We want you to know how to think for yourself!</div>
</div>

Programming is **trial and error**. You shouldn't expect to get it on the first try. And that's OK -- it's part of the process. 

Challenge yourself to understand *why* something went wrong. You'll often find it's because you haven't proof-read your code carefully enough.

<div class="panel panel-success">
  <div class="panel-heading">
<h3 class="panel-title">Read carefully!</h3>
  </div>
  <div class="panel-body">
Take the time to *synthesise* and *integrate* information so that you can apply it to *new scenarios* -- that is the learning objective of your entire university degree. For example, what we do here builds upon everything you've done in this module!

Properly read and digest the material you are given because almost all your modules will test your reading comprehension and attention to detail. And there's nothing in this practical that wasn't covered earlier in semester and in the lectures.

This practical will get progressively more challenging so practice the basics before you progress to harder stuff or answer the assessment.
...
Try to enjoy the moment. Don't overthink it. </div>
</div>
...
<div class="panel panel-warning">
  <div class="panel-heading">
<h3 class="panel-title">Be prepared!</h3>
  </div>
  <div class="panel-body">
...
Everything we do here is building on and applying what we've previously covered. This practical is meant to be challenging so don't stress yourself by attempting the assessment unprepared. </div>
</div>
...
Recall, we expect an initial *positive* relationship between prey density and the number of prey captured -- the number of prey captured will increase with prey density until there are too many prey for a predator to handle and the number of prey captured will plateau. 

We also expect this positive relationship when we take the inverse of our variables -- the linearised data. So "positive" needs to be in our hypothesis but we need to decide the nature of the relationship between the two predictors.
...
<h3 class="panel-title">Info!</h3>
  ...
  <div class="panel-body">
If you needed to change the order of the sub-groups you will need to change the variable to a factor (`as.factor`) and define the order of levels using `levels = c("level1", "level2")`.  
...
```{r dummy}
question_radio(
  "If we reversed the level of colour (`sp`) in the `crabs` dataset from the default, what is the dummy variable of Blue crabs?",
  answer("0"),
  answer("1", correct = TRUE, message = "Orange is now the baseline."),
  allow_retry = TRUE,
  incorrect = "What is the order of the levels now?"
)
```
...
quiz(
  question_radio(
    "Which of the graphs above represents the null hypothesis about crab size?",
    answer("Figure A"),
    ...
    answer("Figure D"),
    allow_retry = TRUE,
    incorrect = "What do the characteristics of the line(s) mean in words?"
  ),
  question_radio(
    "Which of the graphs above represents the alternative hypothesis about crab size?",
    answer("Figure A"),
    answer("Figure B"),
    answer("Figure C", correct = TRUE, message = "The lines have the same slope and different intercepts"),
    answer("Figure D"),
    allow_retry = TRUE,
    incorrect = "What do the characteristics of the line(s) mean in words?"
  )
...
We don't just need to know the difference, we need to statistically test this; whether an estimated/observed value of a sample is significantly different to a known population value. You've leant about this kind of test already.
...
```{r fixed-test}
quiz(caption = "Look at the information for $\\beta_2$, the difference in the intercepts. Answer the following questions based on the R output:",
...
<h3 class="panel-title">Info!</h3>
  ...
  <div class="panel-body">
The one sample t-test is done on each coefficient. The hypotheses are are same. The test can also be phrased as "Does the coefficient explain a significant amount of additional variation in $Y$?".
...
quiz(caption = "Based on the ANOVA table for the additive model on the crabs data:",
     question_text(
       paste("What is the total amount of varation in the data? To one decimal place."),
       answer(paste(10085.3), correct = TRUE, message = random_praise()),
       allow_retry = TRUE,
       incorrect = "SSY is the sum of all SS."
     ),
     question_text(
       paste("What is the F statistic of shell length? To 2 decimal places."),
...
<h3 class="panel-title">Info!</h3>
  ...
  <div class="panel-body">
You may see that the `crabs` dataset with a continuous and categorical predictor variable is analysed using an Analysis of Covariance (ANCOVA). ANCOVAs and ANOVA are simply another way of expressing a linear regression. The additive model we've done here is the same as an ANCOVA. </div>
</div>
...
<h3 class="panel-title">Be comprehensive!</h3>
  ...
  <div class="panel-body">
R output **should not** be presented as a figure or screenshots. Do not copy and paste directly into a report -- this output is meaningless to a reader because they do not know your data like you do or your workflow. 
...
question("Based on the summary of the linear regression you just did, which is the most appropriate sentence reporting the results?",
         answer("There is an effect of shell width or colour on shell length (Fig. 1, linear regression, $t_{197}$ = 16, P < 0.01)", correct = TRUE, message = random_praise()),
                  answer("There is an effect of shell width or colour on shell length (Fig. 1, linear regression, $t$ = 16, P < 0.01)", message = "Does this include all ther information to understand the analysis?"),
         answer("There is an effect of shell width or colour on shell length (Fig. 1, $t_{197}$ = 16, P < 0.01)", message = "Does this include all ther information to understand the analysis?"),
         answer("There is an effect of shell width or colour on shell length (Fig. 1, P < 0.01)", message = "P values are never reported on their own"),
         allow_retry = TRUE,
         random_answer_order = TRUE,
         incorrect = random_encouragement()
         )
...
Notice how the sentence does not include *significant* in the wording. Statistical significance or not is already implied by the wording of the sentence and the inclusion of the P value. The term can also be ambiguous in meaning.
...
You can copy the code in this tutorial and **modify as appropriate for your data**. </div>
</div>
...
These are fundamental R skills. </div>
</div>
...
Make sure you're not making basic errors like using the wrong column name. </div>
</div>
...
<h3 class="panel-title">Hint</h3>
  ...
  <div class="panel-body">
The workflow is exactly as we did for `crabs` -- so you've practised it already (ideally). Remember that R partitions variation sequentially and in alphabetical order for factors.

If you didn't understand the previous material or you got some questions incorrect or you skipped bits, I recommend finishing them before progressing. Demonstrators cannot help you answer the assessment. </div>
</div>
...
Now, both $\beta_0$ and $\beta_1$ vary according to colour, where:
$$\beta_{1_{Blue}} = \beta_1$$
$$\beta_{1_{Orange}} = \beta_1 + \beta_3$$
...
<h3 class="panel-title">Info!</h3>
  ...
  <div class="panel-body">
The structure of the `lm()` function for an interactive model is the same as the additive model but now the relationship between the predictor variables is denoted by `*` to indicate the interaction instead of `+`.</div>
</div>
...
```
```{r random-hint-2}
Do you think it's a + or a * between the predictors?
```
...
Blue is alphabetically before orange so R calculates the variances for blue crabs first, then orange crabs.</div>
</div>
...
<div class="panel panel-info">
  <div class="panel-heading">
<h3 class="panel-title">Time to apply what you've learnt</h3>
  </div>
  <div class="panel-body">
Write down the parameterised equation of the interactive regression for blue and orange crabs before progressing.

Here's the partial regression equation:
$$ shell \space length = \beta_{0_{colour}} + \beta_{1_{colour}} shell \space width + \varepsilon$$</div>
</div>
...
<div class="panel panel-success">
  <div class="panel-heading">
<h3 class="panel-title">Good news!</h3>
  </div>
  <div class="panel-body">
If you could get the equations above, then amazing! That's about as hard as it gets. 

Parameterising the equation is simply knowing what the coefficients represent and knowing which ones to add together. The rest is being able to *use* the equations and interpret the R output. </div>
</div>
...
* `CW:spO` -- This tests whether the slope for orange crabs is different to blue crabs
    ...
    * If this is non-significant, then a multiplicative model is overly complex to describe our biological pattern (because it uses too many parameters) and perhaps an additive model is a better descriptor. 
    
<div class="panel panel-primary">
<div class="panel-heading">
<div class="panel-title">
Aiming to minimise the number of parameters used in a linear model is the statistical concept of **model parsimony**. </div>
</div>
</div>
...
quiz(caption = "Answer the following questions:",
  question_radio(
    "Based on the test on the $\\beta_3$ coefficient above, would you accept or reject the null hypothesis?",
    answer("Accept"),
    answer("Reject", correct = TRUE, message = "The P value is less than 0.05"),
    allow_retry = TRUE,
    incorrect = random_encouragement()
...
*Use what you've learnt to make a graph for the `crabs` dataset showing the regression lines of the interactive linear regression*
```{r multi-graph, exercise = TRUE, exercise.lines = 10}
 ...
 * Parametrised a linear regression in R with biological data
