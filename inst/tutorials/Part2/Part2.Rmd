...
You can remind yourself of how the Type II functional response model is derived from the documentation file - run `vignette("TypeIImodels")` in Console.
...
**Plot your graph of pathogen captured against pathogen density.**  
...
```
This line of code calculates the inverse of `Ha`, that is, 1 divided (`/`) by the number of pathogens destroyed (`Ha`), then saves that number to a new column in our dataset called `Ha.1`. Note no spaces.
...
Since we have done some division, it's a good time to check for any undefined values in case we divided by 0. In `R` this is denoted as infinities (`Inf`). You can check how many infinities there are using `table(is.infinite(class_data$Ha.1))`.

We can replace the infinities with zeroes using `class_data$Ha.1 <- ifelse(class_data$Ha.1 == Inf, 0, class_data$Ha.1)`. You might recognise the if else statement and understand what's happening from previous lectures. We don't discard these observations because a value of 0 has biological meaning.  
...
If `lm` is the function to do a linear regression, `Ha.1` is the name of our response variable, `HT.1` is the name of the predictor variable and the name of our dataset is `class_data`, you should be able to write the code for the linearised type II functional response. **We will ignore vaccination status for now.**
...
Let's do the same thing with our infection model and predict how many pathogens are destroyed by our killer T cell for a given pathogen density using our parameterised model. 
...
Remember our infection response dataset has a **categorical predictor variable** for the vaccination status of an individual. We ignored this in the analyses above meaning we have **pooled** observations for vaccination status.

By pooling vaccination status, any variation in the data generated by vaccination status is **unaccounted for**. Unaccounted variation may increase our chance of making a Type I or II error because vaccination status **confounds** the effect of pathogen density. Phrased differently, the effect of pathogen density on numbers of pathogens destroyed is **masked** by the effect of vaccination status. Thus, in this case, using a simple linear regression on a dataset with two or more predictor variables is not the best course of action.

If we want to see whether vaccination state has an effect on our infection response, we need to include the variable in our linear model.
...
The unvaccinated treatment is coded as `no` and the vaccinated treatment is coded as `yes`. We can also think of this as the unvaccinated group as our experimental control and the vaccinated group as our experimental treatment. 
...
At the moment, vaccination status is classified as a **character** vector (see `str(class_data)`) and, fortunately, our "control" group  (`no`) is alphabetically before `yes`, which makes sense so we don't have to do anything about it. Under the hood, R will convert the `vaccination_status` variable from a character vector to a **factor** when fitting the linear regression. Both characters and factors are ways R stores categorical data. We could define vaccination status as a factor manually to be sure when preparing the data.

If `yes` was first and used to fit the ordinary least squares regression, the estimated coefficients may be different. With three or more levels, especially ordered categories, we may need to make sure R recognises the data are factors , and change to a factor if necessary (using `factor()`), and use `relevel()` to make sure the ordering of the categorical data is what we want for analysis. For example try `factor(c("small", "medium", "large"), levels = c("small", "medium", "large"))` with and without the `levels` argument and see how R orders the levels.
