...
We need to fit a linear model to this data. Remember linear models with two predictor variables take the general form:
...
lm(Y ~ A + B, data)
```
where `A` & `B` are the two predictor variables and `+` indicates the relationship between the two predictors - in a fixed slopes model this is *additive* - `lm` stands for linear model, `Y` is our response variable and `data` is the name of our dataset. `~` indicates a relationship between our response and predictor variables.
...
quiz(
  ...
  question(
    "The value for both slopes are positive. What does that mean?",
