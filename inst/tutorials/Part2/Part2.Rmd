...
subtitle: "2021"
description: "2021 Practical 5 (remote and in-person)"
---
...
**I do not recommend skipping the theory before attempting the CA - these sections will show you all the steps required for the CA**. The demonstrators aren't allowed to help you answer the CA directly but they can help you understand the theory parts so that you can answer the CA. There are no CA questions associated with Parts C & D but that doesn't mean you should skip it because they allow you practise scientific writing and get feedback from the demonstrators.
...
Then, check whether it's a minor thing like a spelling mistake, or a missing quotation mark or bracket, whether your cases don't match, or whether your dataset name and column names exactly match the code. 
...
question(
  ...
  answer("Spinosaurus"),
  random_answer_order = TRUE,
  incorrect = "Incorrect. Have you read the previous section text carefully?"
)
```
...
> * Don't write your code directly in the console either - you won't have a good record of what you've done (in case you wanted to use the code here to help you with your final report or if you make a mistake and have to start again). 
> * You can copy the code in this tutorial and modify as appropriate. There is no certainty that the code here will work right out of the box. The examples here use mathematical notation as the variable names here but the class dataset uses the names from Part 1. You should know which notation matches with what variable description.
...
quiz(caption = "In case you forgot",
     ...
     question("What is does $T_h$ represent?",
          ...
          question("What is does $T_{total}$ represent?",
              answer("Prey density"),
              answer("Time spent searching between prey items"),
              answer("Time spent handling prey"),
...
If `lm` is the function to do a linear regression, `Ha.1` is the name of our response variable, `HT.1` is the name of the predictor variable and the name of our dataset is `class_data`, you should be able to write the code for the linearised type II functional response (or replacing the respective components with the column name and dataset names you are using). **We will ignore foraging strategy for now.**
...
2. Interactive model (**Random slopes or multiplicative model**)
...
Now the intercept parameter ($\beta_{0_{colour}}$) specifies that it is dependent on the colour of the crab. $\varepsilon$ is the random error (residual error) not accounted for by the model - it's always part of the equation but it's often ignored, much like $c$ in integration.

In effect, we are fitting *two* lines to this data - one for each level of colour - the technical term for each of these lines is a **partial regression line**.  

> In additive models the effect of one predictor on the response variable is **additive** of the other (hence they are also called additive models).
...
**But** remember we are expecting *two* lines in our model. **Which line are these coefficients referring to?**
...
At the moment, foraging strategy is classified as a **character** vector (see `str(class_data)`) and, our "control" group (`yes_lid`) is alphabetically after `no_lid`, so we have to do something about it. Under the hood, R will convert the `foraging_strategy` variable from a character vector to a **factor** when fitting the linear regression. Both characters and factors are ways R stores categorical data.

We will not do anything about this for now. R will use `no_lid` as the baseline contrast for $\beta_0$ and $\beta_1$. If you needed to change the order of the sub-groups you will need to change the variable to a factor (`as.factor`) and define the order of levels using `levels = c("level1", "level2")`.
...
> The sturcture of the in a `lm()` function is the same as the additive model but now the relationship between the predictor variables is denoted by `*` to indicate the interaction. 

It's time to do the linear model. You should now be comfortable with this process because it is the same as the simple regression and the additive model.
...
In the additive model we only needed to manually calculate the intercept. Now we need to manually calculate the slope **and** intercept.  
...
Parameterising the intercept is exactly the same as the additive model - `(Intercept)` and `spO` mean the same thing as before - the estimated intercept and the difference in the estimated intercept between blue and orange crabs. So we should be able to do this right away.
...
> The coefficient estimate `CW:spO` is the **difference in the value of the slope for orange crabs relative to the slope for blue crabs** `CW`
...
question_text(
  ...
  answer(paste(round(coef(crabs_int)[2],2) + round(coef(crabs_int)[4],2)), correct = TRUE),
  allow_retry = TRUE,
...
Here's the `summary()` of our multiplicative model
...
Here are the coefficients again for our simple linear model for the crabs showing the intercept and slope respectively:
```{r coef-reminder}
