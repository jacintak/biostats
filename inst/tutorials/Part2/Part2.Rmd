...
1. A step by step recap of the lecture content and the practicalities of linear regression in R using a worked example. Work through the coding exercises and answer the questions *in order*.

2. Applying your understanding of the content from the example to the data you collected last practical. This is the assessment.
...
<h3 class="panel-title">Danger!</h3>
  ...
  <div class="panel-body">
**1. Do not take the code here for granted** 
...
You can **and should** easily fix common coding typos yourself!  
We want you to know how to work problems out independently.

***

**2. Do not skip questions or exercises to answer your assessment on the fly**
...
If something does not look correct, first check that you have not missed a step.
</div>
</div>
...
Now we can fit a linear model where the response variable is $\frac{1}{H_a}$ and the predictor variable is $\frac{1}{H \times T_{total}}$, and find the values of search rate ($a$) and handling time ($T_h$) from the slope and intercept of the linear regression.
...
*Enter the code to check the `summary()` of the __additive linear regression__ for the `crabs` dataset:*
```{r fixed-summary, exercise = TRUE}

```
```{r fixed-summary-hint-1}
Check the summary of the model with summary(). What goes inside summary()? We are not looking for summary statistics of the dataset
```
...
Remember the graph looks like this now:

```{r tyepe2line2, echo = FALSE, fig.cap= "Linearised type II functional response of an predator-prey response"}
prey <- seq(0, 0.25, 0.01)
type2 <- seq(0, 1, length.out = length(prey))
plot(type2 ~ prey , type = 'l', lwd = 4, ylab = "1/Ha, Prey captured", xlab = "1/H Total time, Prey density",
     xaxt='n', yaxt='n')
