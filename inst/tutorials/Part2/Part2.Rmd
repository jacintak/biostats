...
test_data <- data.frame(concentration = seq(1,20))
```
...
<div class="panel-heading">
<div class="panel-title">
We have covered all of these concepts in the lectures or previous practicals. This prac will reinforce and revise these concepts and allow you to practise them under supervision from the demonstrators. </div>
</div>
</div> 
...
<div class="panel-heading">
<div class="panel-title">
Don't forget to answer the CA questions in a separate link. There are prompts that will tell you when to answer the questions. </div>
</div>
</div> 
...
<div class="panel-heading">
<div class="panel-title">
I do not recommend skipping the theory before attempting the CA - the theory section will go through all the steps you need for analysing your data. </div>
</div>
</div>
...
You should aim to understand all the code here and describe what it's doing. </div>
</div>
...
<h3 class="panel-title">Let me google that for you</h3>
  ...
  <div class="panel-body">
There's no shame in googling everything - we do too and finding things out yourself part of learning to work independently or troubleshooting. </div>
</div>
...
question_text("What is the median of concentration?",
  answer(paste(median(test_data$concentration)), correct = TRUE, message = random_praise()),
  allow_retry = TRUE,
  incorrect = random_encouragement()
)
```
...
In R, both the order of the predictor variables and the order of the sub-groups of a categorical variable are relevant because this is the order R partitions variation in the response variable to the predictor variables.
...
question_radio(
  ...
  allow_retry = TRUE,
  incorrect = "How are dummy variables assigned?"
)
```
...
lines(seq(0, 0.6, length.out = length(prey)) ~ prey, lwd = 4, col = 2)
legend("topleft", bty = "n", c("Group 1", "Group 2"), lwd = c(4, 4), col = c(3,2))
```
...
We could have multiple hypotheses that also test whether there *is* a trend in the data but the specific wording of our hypotheses above do not do so.
...
<h3 class="panel-title">Time to apply what you've learnt</h3>
  ...
  <div class="panel-body">
Write down the parameterised equation of the interactive regression for blue and orange crabs before progressing.</div>
</div>
