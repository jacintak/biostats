...
lm(Y ~ X1 + X2, data)
```
where `X1` & `X2` are the two predictor variables and `+` indicates the relationship between the two predictors - in an additive model this is *additive* - `lm` stands for linear model, `Y` is our response variable and `data` is the name of our dataset. `~` indicates a relationship between our response and predictor variables.
...
We won't plot our regression lines just yet but you should be able to interpret the R output as a graph by looking at the coefficient values. We will construct a graph in Part B of the practical!
