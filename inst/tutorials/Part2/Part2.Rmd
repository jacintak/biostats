...
> Don't forget to answer the CA questions in a separate link. I recommend answering the relevant questions as you progress along the prac when prompted but don't submit until the end. 
...
It will help to plot the graph.
...
1. Additive model 
2. Interactive model (**Multiplicative model**)

The difference is in how they describe the slope and the intercept of the linear regression. **This changes the interpretation of the model and the associated hypotheses.**
...
Where $X_1$ is our first predictor variable and $X_2$ is our second predictor variable. $\beta$ are the regression coefficients (also called terms) describing the variation in $Y$ attributed to each coefficient. $\varepsilon$ is the random error (residual error) not accounted for by the model - it's always part of the equation but it's often ignored, much like $c$ in integration.

 > The model is called a "**full**" model because it contains all possible terms  - all predictor variables and all possible combinations of their interactions.
...
To use the model, we need to **parameterise** the model, meaning we need to estimate the values of the four model terms (regression coefficients): $\beta_0$, $\beta_1$, $\beta_2$ and $\beta_3$. This is R's job.
...
The general function to plot a graph in `R` is `plot(Y ~ X, data)` replacing `X`, `Y` and `data` with the respective information from your dataset. By default, the graph will plot individual points. We will plot a more detailed figure later in the practical.
...
> The sturcture of the `lm()` function for an interactive model is the same as the additive model but now the relationship between the predictor variables is denoted by `*` to indicate the interaction instead of `+`. 
...
Parameterising the intercept is exactly the same as the additive model - `(Intercept)` and `spO` mean the same thing as before - the estimated intercept for the baseline group ($\beta_0$) and the difference in the estimated intercept between blue and orange crabs ($\beta_2$). You've done this!
...
We can look at the ANOVA table for an additive or interactive linear model with `anova(lm())`. If it's an interactive model the amount of variation in shell length attributed to the interaction between colour and shell width will also be displayed (sum of squares).
...
Time to use what you've learnt about interactive linear models on your dataset and parameterise $Y = \beta_{0_{foraging \space strategy}} + \beta_{1_{foraging \space strategy}} X_1$!
...
That's multiple regression. You should now know how to find the values of $\beta$ in $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_1 X_2 + \varepsilon$.
The parameterising process is similar to simple regression but the additional predictor variable means we need to put a bit more work in. Once you have your parameters, the rest is straightforward algebra. Well done!
...
> There are no more CA questions. Make sure you have submitted your answers
