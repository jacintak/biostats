...
subtitle: "2022"
description: "2022 Practical 5 (remote and in-person)"
---
...
Part A: Using multiple linear regressions (models) in R (80 mins)  
**Learning objectives:**
...
We will skip simple linear regressions because the same concepts and process apply to simple regressions as multiple regressions.

Part B: Visualising data (30 mins)  
**Learning objectives:**
...
Part C: Summarising results in text (20 mins)  
**Learning objectives:** 
...
question(
  ...
  answer("Spinosaurus"),
  random_answer_order = TRUE,
  incorrect = "Incorrect. Have you read the worksheet carefully?"
)
```

***

## Part A: Multiple linear regressions
...
In effect, we are fitting *two* lines to this data - one for each level of colour - the technical term for each of these lines is a **partial regression line**. Together they make up the full model above.
...
lm(Y ~ A + B, data)
```
where `A` & `B` are the two predictor variables and `+` indicates the relationship between the two predictors - in an additive model this is *additive* - `lm` stands for linear model, `Y` is our response variable and `data` is the name of our dataset. `~` indicates a relationship between our response and predictor variables.
...
2. Coefficients are the estimated coefficients of the model. Let's start with the first two coefficients. The interpretation for these coefficients is the same as simple linear regression. There's the intercept `(Intercept)` and there's the slope `CW`.  
...
So, we already have our equation for blue crabs. Parametrised equation for blue crabs:  
...
For example, using the `crabs` example, we can use the parametrised equation of a simple linear regression (ignoring colour), $CL =$ `r round(coef(crabs_lm)[1],1)`  + `r round(coef(crabs_lm)[2],1)` $\times CW$, to work out the length of a crab for any value of shell width. 
...
*Fix this linear model function to do a multiplicative linear regression in R. There is more than one mistake. Press run.*
...
## Part C: Summarising results in text  
