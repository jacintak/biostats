...
In this practical we aim to take a practical and hands-on approach to learning statistical modelling by using R and a real dataset you collected. We focus on the practicalities of *applying* statistics (implementation & interpretation) - not on the theory or mathematics.
 ...
 * Know how to parameterise a linear regression from R output 
...
1. A theory section where we will go over the practicalities of linear regression in R step by step using an example dataset. There are code chunks and revision questions. You can retry questions you get wrong - learning is about the process not the marks!
2. A practice section where you will apply your new skills to the data you collected last practical. This is required to answer the CA for Parts A & B. You will need access to the class dataset from the previous practical so save it somewhere you know. I do not recommend skipping the theory before attempting the CA. The demonstrators aren't allowed to help you answer the CA directly but they can help you understand the theory parts so that you can answer the CA. There are no CA questions associated with Parts C & D but that doesn't mean you should skip it because they allow you practise scientific writing and get feedback from the demonstrators.

> Don't forget to answer the CA questions in a separate link. I recommend answering the CA as you progress along the prac when prompted.
...
We saw the statistical theory of linear regressions in the lectures. We will use the dataset `crabs` - see `help(crabs)` for more information. This dataset is provided in `R` in the package `MASS`. I have already loaded the package for you so you don't need to do it.
...
The mathematical expression for the blue line is:
...
### Practice: Parameterising a linear model of infection response

Remember we had some unknown parameters in our infection functional response model? The search rate, $a$, and the handling time, $T_h$. We need to find the value of those parameters from the slope and the intercept of our linearised Type II dataset - exactly like we did for the `crabs` example. We can use the `lm` function on our infection model data to parameterise our model.
...
> * You can copy the code in this tutorial and modify as appropriate.
...
```
This imports the spreadsheet into an R object called `class_data`.

You need to replace the file address within the quotation marks with where ever you saved your file on your computer. File housekeeping is important!
...
quiz(caption = "In case you forgot",
     question("What is does $H_a$ represent?",
              ...
              answer("Pathogens destroyed", correct = TRUE),
              answer("Number of killer T cells"),
              answer("Duration of the infection"),
              allow_retry = TRUE),
     question("What is does $H$ represent?",
              answer("Pathogen density", correct = TRUE),
     ...
     question("What is does $T_h$ represent?",
              ...
              answer("Pathogens destroyed"),
              answer("Number of killer T cells"),
              answer("Time spent to destroy one pathogen", correct = TRUE),
              allow_retry = TRUE))
```
...
```
class_data$1.Ha <- 1/Ha
```
This line of code calculates 1 divided by the number of pathogens destroyed (`Ha`), then saves that number to a new column in our dataset called `1.Ha`. Note no spaces.

Now let's linearise the predictor variable (`H`) to calculate $\frac{1}{H\times T_{total}}$. 

```
class_data$1.HT <- 1/(H * T_total)
```
Because we use a value of 1 minute for $T_{total}$, we are essentially dividing 1 by only pathogen density ($H$). Told you a value of 1 would make our maths easier!

Since we have done some division, it's a good time to check for any undefined values in case we divided by 0. In `R` this is denoted as infinities (`Inf`). We can replace the infinities with zeroes using `class_data$1.Ha <- ifelse(class_data$1.Ha == Inf, 0, class_data$1.Ha)`. You might recognise the if else statement and understand what's happening from previous lectures. We don't discard these observations because a value of 0 has biological meaning.  

We now have all the correct columns to parameterise our functional response.

> Your column names may not be exactly the same but you can apply the same process using your column names. E.g. you don't *have* to call it `1.HT` but recognise that you need to remember to modify your code accordingly.

**Step 3: Doing the linear regression**  
If `lm` is the function to do a linear regression, `1.Ha` is the name of our response variable, `1.HT` is the name of the predictor variable and the name of our dataset is `class_data`, you should be able to write the code for the linearised type II functional response. **We will ignore vaccination status for now.**

(Hint: see the theory section above if stuck)

```{r error, echo = FALSE}
quiz(caption = "Getting an error message?",
     question("What gets recorded if the killer T cell did not catch any pathogens?",
...
> Linear regressions are always done on the entire data, not on averages. i.e. you wouldn't use the average of your replicates for each treatment for the underlying data. Ordinary least squares regression depends on quantifying *variation* of observations around the mean for fitting a line to data. So averaging data removes information for `R` to use (degrees of freedom).

**Step 4: Calculating $a$ and $T_h$**  
The final step is to get values of $a$ and $T_h$ since we know from our functional response:

$$Slope=\ \frac{1}{a}$$  
and
$$Intercept=\ \frac{T_h}{T_{total}}$$  

You can rearrange these equations to solve for $a$ and $T_h$

That's all! You should now have all the values needed for a parameterised type II model.
...
We can calculate the value of the response variable from any given value of the predictor variable. You need a fully parameterised model to do this - which we do. 
...
Let's do the same thing with our infection model and predict how many pathogens are destroyed by our killer T cell for a given pathogen density. 
...
*Do you know of one from previous lectures?*
...
The P value of the one sample t test on the slope of our linear regression is < 0.05, meaning that there is less than 5% chance that our estimated slope is not significantly different to 0. Thus, we can **reject the null hypothesis and accept the alternative hypothesis**:
...
Let's remind ourselves what the hypotheses for the infection model are from the previous practical:
...
Now you can look at the summary of your linear regression on the infection response from earlier and evaluate these hypotheses based on the P value of the slope coefficient.
...
> Take a break. Stand up. Shake your limbs. Breathe.
...
The difference is in how they calculate the slope and the intercept of the linear regression. **This changes the interpretation of the model.**
...
Now the intercept parameter ($\beta_{0_{colour}}$) specifies that it is dependent on the colour of the crab.  
In effect, we are fitting *two* lines to this data - one for each level of colour - each of these lines is technically called **partial regression lines**.  
...
question_radio(
  ...
  answer("Blue crabs", correct = TRUE, message = "B comes before O. It also says spO in the next coefficient which tells you that is for orange crabs"),
  answer("Orange crabs"),
  allow_retry = TRUE
)
```

So the first two coefficients are the intercept and slope of the linear regression for blue crabs.
...
```{r predict-fixed}
blue <- sample(10:70,1)
ans_blue <- round(crabs_add$coefficients[1],2) + round(crabs_add$coefficients[2]*blue, 2)
orange <- sample(10:70,1)
ans_orange <- round((crabs_add$coefficients[1] + crabs_add$coefficients[3]),2) + round(crabs_add$coefficients[2]*orange, 2)
###
size <- sample(10:70,1)
size_blue <- round(crabs_add$coefficients[1],2) + round(crabs_add$coefficients[2]*size, 2)
size_orange <- round((crabs_add$coefficients[1] + crabs_add$coefficients[3]),2) + round((crabs_add$coefficients[2] + crabs_add$coefficients[4]) * size,2)
ans_size <- size_orange - size_blue

###

quiz(
  question_text(
    paste("What is the carapace length of an orange crab with a carapace width of", blue, " mm? To 2 decimal places"),
    answer(paste(ans_blue), correct = TRUE),
    allow_retry = TRUE
  ),
  question_text(
    paste("What is the carapace length of a blue crab with a carapace width of", orange, " mm? To 2 decimal places"),
...
quiz(
  ...
  question_radio(
    ...
    answer("There is no effect of shell width or colour on the response variable"),
    answer("There is an effect of shell width or colour on the response variable", correct = TRUE),
    allow_retry = TRUE
  )
)
```
...
### Practice: Evaluating infection responses with additive models

Remember our infection response dataset has a **categorical predictor variable** for the vaccination status of an individual. We ignored this in the analyses above so we have **pooled** observations for vaccination status.

If we want to see whether vaccination state has an effect on our infection response, we need to include the variable in our linear models.
...
Parameterising the intercept is exactly the same as the fixed slopes model - `(Intercept)` and `spO` mean the same thing - so we should be able to do this right away.
...
quiz(
     question_text(
       ...
       answer(paste(coef[1]), correct = TRUE, message = "(Intercept) refers to the intercept for blue crabs"),
       allow_retry = TRUE
     ),
      question_text(
...
```{r predict-random}
blue <- sample(10:70,1)
ans_blue <- round(crabs_int$coefficients[1],2) + round(crabs_int$coefficients[2]*blue, 2)
orange <- sample(10:70,1)
ans_orange <- round((crabs_int$coefficients[1] + crabs_int$coefficients[3]),2) + round((crabs_int$coefficients[2] + crabs_int$coefficients[4])* orange,2)
###
size <- sample(10:70,1)
size_blue <- round(crabs_int$coefficients[1],2) + round(crabs_int$coefficients[2]*size, 2)
size_orange <- round((crabs_int$coefficients[1] + crabs_int$coefficients[3]),2) + round((crabs_int$coefficients[2] + crabs_int$coefficients[4]) * size,2)
ans_size <- size_orange - size_blue
###
...
quiz(
  ...
  question_radio(
    "Based on the test on the intercept coefficient above, would a fixed slopes model or random slopes model be more parsimonious to describe the effect of colour and shell width on shell length?",
...
### Practice: Evaluating infection responses with multiplicative models

We'll rephrase our hypotheses slightly to make the effect of the interaction explicit:    
H0: The relationship between pathogens destroyed and pathogen density is the same between vaccinated and unvaccinated individuals    
H1: The relationship between pathogens destroyed and pathogen density is different between vaccinated and unvaccinated individuals
...
> Do a happy dance. Pat yourself on the back.  
> There are no more CA questions.
...
> Red-Green colour blindness is the most common form of colour blindness. A simple guideline is to avoid using red and green together where ever possible. Blue and orange are better constrasting colours (that's why they are common colours for movie posters).
...
```{r col, exercise=TRUE, exercise.lines = 5}
plot(CL ~ CW, crabs, type = "n")
points(CL ~ CW, crabs[crabs$sp == "B",])
points(CL ~ CW, crabs[crabs$sp == "O",])
```
...
Shapes are important because it makes the points stand out. It can also be used to distinguish between groups of data on the same graph. Sometimes it's necessary to change colour *and* shape to make it easier to distinguish between groups - redundancy is acceptable.
...
```{r pch, exercise=TRUE, exercise.lines = 2}
...
```{r axes, exercise=TRUE, exercise.lines = 2}
plot(CL ~ CW, crabs)
```
```{r axes-hint-1}
...
Here are the coefficients again our simple linear model for the crabs the intercept and slope respectively:
```{r coef-reminder}
...
```{r final-graph-test, exercise=TRUE, exercise.lines = 6}
plot(CL ~ CW, crabs)
```
...
Use your new found knowledge of making graphs in R to make a plot of our results from our infection response including appropriate axes labels with units. The aesthetics are up to you. Write a caption as well.
...
Statistical analyses are always presented in text, never as a figure (i.e. you'll never see a screenshot from R in a professional scientific report or paper). There are standard conventions for presenting test statistics and P values. There may be variation across disciplines but generally look at this sentence:

```
The predictor variable had a positive effect on the response variable (Figure 1, Two-way t-test, t = 3.33, P = 0.56).
```
...
"The slopes of the linear model differed between blue and orange crabs (linear regression, $t$ = 2.07, $P$ = 0.04)."

***

### Practice: Writing a results paragraph

Practice writing a results paragraph using the results from our infection response. You can pick whatever model results to describe. You can check what you've written is comprehensive with a demonstrator. This is an essential skill for writing scientific reports.
...
That's the end of the prac. We've covered a lot of practical skills that you can use in the future and in your other modules. Just as you should build upon what you've learnt in previous modules in this one.
