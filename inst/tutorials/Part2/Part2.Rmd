...
#subtitle: "2022"
description: "2022 BYU22S01 Practical 5"
---
...
```{r test-hint}
/ means divide by in R. You do not need =
```
...
**Quizzes**  
**Quizzes here are not part of the CA.** The questions are meant to check you understand the material and your answers are not visible to anyone else. I don't recommend skipping these. The CA is accessed separately.
...
Although you can come back to this tutorial at any time, I recommend taking your own notes for your reference to supplement your lecture notes. 
...
If you have no trouble with the example dataset or the lecture content, then you'll have no trouble with the CA or your final report.</div>
</div>
</div>
...
<h3 class="panel-title">Discussion</h3>
  ...
  <div class="panel-body">
What would you expect to see graphically for these hypotheses?
</div>
</div>
...
ANOVA tables show the Sum of Squares (SS) of the linear model. This is the variation partitioned to each model coefficient.

> Variance of predictors (SS) is partitioned out from total SS in the order it is entered in to R

We can look at the ANOVA table for an linear model with `anova(lm(<linear model>))`. We expect to see a row for each term in the model, plus the residual variation, $\varepsilon$. From all the Sum of Squares, we can calculate the total variation in the data (SSY) and calculate metrics like the coefficient of determination $R^2$.
...
<h3 class="panel-title">Remember!</h3>
  ...
  <div class="panel-body">
Refer to the previous practicals to revise these basic steps. Make sure your address is correct and don't forget .csv at the end.
